...
require_once JPATH_SITE.'/components/com_truematrimony/models/interests.php';
...
$intermodel = new TruematrimonyModelInterests();

$interitems = $intermodel->getInterests();

$profileinterlist = $intermodel->getProfileInterests();
...
/**
 * Get the profile package information.
 */ 
$ppackageinfo = $model->getProfilePackageInfo();
...
<div class="row-fluid truematrimony-profile">
	...
	<div class="col-md-3">
	   ...
	   <div class="profile-upgrade">
		   <?php
		   $start_date = date_create($ppackageinfo['package_start']);
		   $end_date = date_create($ppackageinfo['package_end']);
		   $durations = date_diff($start_date, $end_date);
		   
		    if(!empty($ppackageinfo)) {
		    ?>	   
				<a href="javascript:void(0)">
				    <?php echo JText::_('COM_TRUEMATRIMONY_PROFILE_END_DATE').date("d-M-Y", strtotime($ppackageinfo['package_end']));?>
				    <?php echo " (".$durations->format('%a Days').")" ?>
				</a>		
					
			    <a href="#viewinterests" data-toggle="modal">
				    <?php echo JText::_('COM_TRUEMATRIMONY_PROFILE_INTERESTED_PROFILES').count($profileinterlist); ?>
			    </a>
			    
			    <a href="javascript:void(0)">
					<?php 
						echo JText::_('COM_TRUEMATRIMONY_PROFILE_REMAINING_INTERESTS');
						echo $ppackageinfo['profile_limit'] - count($profileinterlist);				
					?>
			    </a>
			    
			    <!-- display the all interests profiles -->		    
			    <!-- <a href="#viewinterests" data-toggle="modal">
					<?php //echo JText::_('COM_TRUEMATRIMONY_PROFILE_ALL_INTERESTS_VIEW'); ?>				
				</a> -->		    
			<?php } ?>
	        ...
	        <?php if(!empty($ppackageinfo)) { ?>
				<label class="label label-success profile-current-plan" style="padding:7px 10px; font-size:15px;"><?php echo "Current Plan: ".$ppackageinfo['package_name']; ?></label>
			<?php } ?>
			<?php if($itemval['package_status'] == 1) { ?>
				<label class="label label-warning profile-current-plan" style="padding:7px 10px; font-size:15px;"><?php echo "Pending"; ?></label>
			<?php } else if($itemval['package_status'] == 2) { ?>
			    <label class="label label-success profile-current-plan" style="padding:7px 10px; font-size:15px;"><?php echo "Approved"; ?></label>
			<?php } else if($itemval['package_status'] == 3) { ?>
				<label class="label label-danger profile-current-plan" style="padding:7px 10px; font-size:15px; background:#b71c08;"><?php echo "Discard"; ?></label>
			<?php } ?>		
	  ...
	  <form class="adminForm form-horizontal truematrimony-profile-info" id="adminForm" name="adminForm" method="post" enctype="multipart/form-data">
		...
		<?php
			...
			<div class="upload-files">
				<a href="#uploadfile" data-toggle="modal"><?php echo JText::_('COM_TRUEMATRIMONY_UPLOAD_PROFILE_FILE'); ?></a>
			</div>	
	   ...
	   <form class="adminForm form-horizontal truematrimony-profile-info" id="adminForm" name="adminForm" method="post" enctype="multipart/form-data">
		...
		<!-- modal div starts -->				
		<div class="modal fade container col-md-6" id="uploadfile">
			
			<!-- modal header div -->
			<div class="modal-header">
				<h5><?php echo JText::_('COM_TRUEMATRIMONY_UPLOAD_PROFILE_FILE'); ?></h5>
				<a href="javascript:void(0)" style="float:right;" data-dismiss="modal"><?php echo "X"?></a>
			</div>
			...
			<!-- modal footer -->
			<div class="modal-footer">
				<button type="submit" class="btn btn-warning"><?php echo JText::_('COM_TRUEMATRIMONY_UPLOAD_PROFILE_FILE_SAVE'); ?></button>
			</div>
	...
	<div class="col-md-9">
	  ...
	  <!-- display the all interest profiles modal starts -->			
	  <div class="modal hide fade container col-md-8" id="viewinterests">
		  
		  <form class="adminForm form-horizontal truematrimony-profile-upgrade" id="adminForm" name="adminForm" method="post">
				
				<!-- modal header starts -->
				<div class="modal-header">
					<h4 id="myModalLabel"><?php echo JText::_('COM_TRUEMATRIMONY_PROFILE_ALL_INTERESTS_VIEW'); ?></h4>
					<button type="button" class="btn btn-warning viewinterests-close" data-dismiss="modal"><?php echo "X"; ?></button>
				</div><!-- modal header ends -->
				
				<!-- modal body div starts -->
				<div class="modal-body">
					<!-- profile interests unorder list starts -->
					<ul class="interest-profiles-ul">
						<?php for($i=0; $i<count($profileinterlist); $i++) { ?>
										
						<li class="row-fluid interest-profiles-lists">
																					
							<!-- interest profile picture div starts -->
							<div class="span3 col-md-3">								
								
								<!-- profile member default image -->
								<?php
									if($profileinterlist[$i]['profile_img']!="") {			
									$path = JUri::root().'/media/truematrimony/profiles/profile_'.$profileinterlist[$i]['truematrimony_profile_id'].'/thumbs/'.$profileinterlist[$i]['profile_img'];	
									?>
									<img src="<?php echo $path;?>" class="" style="width:120px;"> 	
									<?php	   
									} else {
										if($itemval['gender']==1) {
										   $path = JURI::root().'/components/com_truematrimony/helpers/images/no-female.png'; ?>
									...
									?>
							</div><!-- interest profile picture div ends -->
							
							<!-- interest profile informations div starts -->
							<div class="span9 col-md-9">							
							
							<!-- interest profile id div starts -->
							<div class="inter-profiles span9">
								<label class="control-label"><?php echo JText::_("COM_TRUEMATRIMONY_PROFILE_ID"); ?></label>
								<div class="control-groups">
									<?php
									if($itemval['gender']==1) {
										echo "F".$profileinterlist[$i]['truematrimony_profile_id'];		
									} else {
										echo "M".$profileinterlist[$i]['truematrimony_profile_id'];
									}									
									?>								
								</div>	
							</div><!-- interest profile id div ends -->	
														
							<!-- interest profile name div starts -->
							<div class="inter-profiles span9">
								<label class="control-label"><?php echo JText::_('COM_TRUEMATRIMONY_PROFILE_NAME'); ?></label>
								<div class="control-groups">
									<?php echo $profileinterlist[$i]['profile_name']; ?>
								</div>	
							</div><!-- interest profile name div ends -->	
							
							<!--interest profile date of birth div starts -->
							<div class="inter-profiles span9">
								<label class="control-label"><?php echo JText::_('COM_TRUEMATRIMONY_PROFILE_DOB'); ?></label>
								<div class="control-groups">
									<?php echo $profileinterlist[$i]['dob']; ?>
								</div>	
							</div><!--interest profile date of birth div ends -->
							
							<!-- interest profile age div starts -->
							<div class="inter-profiles span9">
								<label class="control-label"><?php echo JText::_('COM_TRUEMATRIMONY_PROFILE_AGE'); ?></label>
								<div class="control-groups">
									<?php echo $profileinterlist[$i]['age']; ?>
								</div>	
							</div><!-- interest profile age div ends -->						
						  </div><!-- interest profile informations div ends -->																						
						</li>
						<?php } ?>
					</ul><!-- profile interests unorder list ends -->	
					
				</div><!-- modal body div ends -->
				
				<!-- modal footer div starts -->
				<div class="modal-footer">
					<button type="button" class="btn btn-warning" data-dismiss="modal"><?php echo JText::_('COM_TRUEMATRIMONY_MODAL_CLOSE'); ?></button>
				</div><!-- modal footer div ends -->			
          
		  </form>
		  
	  </div><!-- display the all interest profiles modal ends -->	
	  ...
	  <div class="modal hide fade container col-md-8" id="upgradeprofile" style="background:#ffffff;">	
		...
		<form class="adminForm form-horizontal truematrimony-profile-upgrade" id="adminForm" name="adminForm" method="post">
					...
					<ul class="profile-plan-ul">
						...
						<li>
						 	<ul class="profile-plan-ul-head list-groups span12">
								<li class="list-group-item span2"><h5><?php echo JText::_("COM_TRUEMATRIMONY_PROFILE_UPGRADE_PLAN_PACKAGE_HEAD");?></h5></li>
								...
								-->
								<li class="list-group-item span2"><h5><?php echo JText::_("Duration"); ?></h5></li>
						...
						<?php 					
							...
							$packageinfo = JArrayHelper::fromObject($package);
						?>				
						...
						<li>
							<ul class="profile-plans-ul span12">
								...
								<!-- mankalan plan list -->
								<li class="span2"><span class="profile-upgrade-plan"><?php echo $packageinfo['package_name']; ?></span></li>
								<li class="span2"><span><?php echo 'Rs.'.$packageinfo['package_cost']; ?></span></li>
								<!-- <?php if($packageinfo['package_month']!=0) :?>
								<li class="span2"><span><?php echo $packageinfo['package_month'].' Months'; ?></span></li>
								<?php endif; ?>
								<?php if($packageinfo['package_year']!=0) { ?>
								<li class="span2"><span><?php echo $packageinfo['package_year'].' Years'; ?></span></li>
								<?php } else if($packageinfo['package_year']=="Lifetime") {?>
								<li class="span2"><span><?php echo $packageinfo['package_year']; ?></span></li>
								<?php } else if(empty($packageinfo['package_month']) && empty($packageinfo['package_year'])) { ?>
								<li class="span2"><span><?php echo " - " ?></span></li>	
								<?php }?> -->
								
								<?php if(!empty($packageinfo['package_start']) && !empty($packageinfo['package_end'])) { 
									$start_date = date_create($packageinfo['package_start']);
									$end_date = date_create($packageinfo['package_end']);
									$durations = date_diff($start_date, $end_date);
								?>	
								<?php	
									if($durations->format('%y')!="0") {
										if($packageinfo['package_lifetime']) { ?>
											<li class="span2"><span><?php echo "Life Time"; ?></span></li>
											<?php }  else { ?>
										     <li class="span2"><span><?php echo $durations->format('%y Years'); ?><?php echo " (".$durations->format('%a Days').")" ?></span></li>
									        <?php } ?>								
									<?php } else if($durations->format('%m')!="0") { ?>
									<li class="span2"><span><?php echo $durations->format('%m Months'); ?><?php echo " (".$durations->format('%a Days').")" ?></span></li>	 
									<?php } 
									}?>								
								<li class="span3"><span><?php echo $packageinfo['profile_limit']. ' Profiles'; ?></span></li>
								<li class="span2">								   					
									<div class="col-md-2">
										<label class="radio-inline">
										  <input type="radio" name="package_id" id="package_id" value="<?php echo $packageinfo['truematrimony_package_id']; ?>">
										</label>									
								    </div>					
								</li>								
							</ul>												
			...
			<input type="hidden" name="option" value="com_truematrimony"/>
			<input type="hidden" name="view" value="member"/>
			<input type="hidden" id="task" name="task" value="updatepackage"/>
			<input type="hidden" name="layout" value="item_member"/>	
			<input type="hidden" name="truematrimony_profile_id" id="truematrimony_profile_id" value="<?php echo $itemval['truematrimony_profile_id']; ?>" >	
